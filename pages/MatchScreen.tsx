...
export default function MatchScreen({ homeTeam, awayTeam, result, playerStates, currentTactics, onTacticsChange, onClose }: Props) {
  ...
  const [speed, setSpeed] = useState<Speed>(loadSettings().matchSpeed as Speed);
  ...
  const tick = useCallback(() => {
    setMinute(prev => {
      if (phase === 'first_half' && prev + speed > 45) return 45;
  ...
  function logoFallback(team: Team) {
    return <TeamLogo teamId={team.id} logoUrl={team.logoUrl} fallbackName={team.name} />;
  }
    ...
    <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col text-white select-none">
      ...
      <div className={`px-4 py-3 flex items-center justify-between transition-colors duration-500 ${headerBg}`}>
        <div className="flex flex-col items-center w-20">
          <div className="w-10 h-10 mb-1">{logoFallback(homeTeam)}</div>
        ...
        <div className="flex flex-col items-center w-20">
          <div className="w-10 h-10 mb-1">{logoFallback(awayTeam)}</div>
      ...
      <div className="px-3 pt-2 pb-1 relative">
        <Pitch2D 
          homeTeam={homeTeam} 
          awayTeam={awayTeam} 
          lastEvent={lastAction ?? null} 
          isPlaying={isPlaying} 
        />
        {/* Placar de tempo integrado */}
        <div className="absolute bottom-1.5 left-4 right-4 h-1 bg-gray-900 rounded-full overflow-hidden opacity-50 z-20">
          <div className="h-full bg-white transition-all duration-200" style={{width:`${(minute/90)*100}%`}}/>
        </div>
        {goalFlash && <div className="absolute inset-x-3 inset-y-2 rounded-xl bg-yellow-400 opacity-30 animate-pulse pointer-events-none z-30"/>}
      </div>
