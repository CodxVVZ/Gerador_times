...
import { teams, LEAGUES, COUNTRIES } from "@/lib/teams";
import { useLocation } from "wouter";
import { useDarkMode } from "@/contexts/DarkModeContext";
import { useGame } from "@/contexts/GameContext";
import { TeamLogo } from "@/components/TeamLogo";
import { loadSettings } from "@/lib/saveSystem";
...
export default function NewGame() {
  const [language, setLanguage] = useState<Language>(() => loadSettings().language as Language);
  const firstLeagueId = Object.keys(LEAGUES)[0] || "";
  const [selectedLeague, setSelectedLeague] = useState<string>(firstLeagueId);
  ...
  const leagueOptions = Object.entries(LEAGUES).map(([id, info]) => ({ id, ...info }));
  const teamsInSelectedLeague = teams.filter(t => t.leagueId === selectedLeague);
    ...
    <div className={`min-h-screen flex flex-col ${
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-8 p-8">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {teams.length === 0 ? (
            <div className={`p-8 rounded-lg border-2 border-dashed text-center ${isDarkMode ? "border-gray-700 text-gray-400" : "border-gray-300 text-gray-500"}`}>
              <p className="mb-4">Nenhum banco de dados carregado.</p>
              <p className="text-sm">Volte para o menu inicial para importar um banco de dados (.emp, .zip ou .json).</p>
            </div>
          ) : (
            <>
              {/* League Selector */}
              <div className="flex flex-wrap gap-2 mb-6">
                {leagueOptions.map(l => {
                  const cInfo = COUNTRIES[l.country];
                  const displayLabel = cInfo ? `${cInfo.flag} ${l.name}` : l.name;
                  return (
                    <button
                      key={l.id}
                      onClick={() => { setSelectedLeague(l.id); setSelectedTeamId(null); }}
                ...
                {teamsInSelectedLeague.map((team) => (
                  ...
                  >
                    <div className="w-12 h-12 mb-2">
                      <TeamLogo teamId={team.id} logoUrl={team.logoUrl} fallbackName={team.name} />
                    </div>
